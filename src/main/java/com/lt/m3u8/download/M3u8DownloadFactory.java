package com.lt.m3u8.download;

import com.lt.m3u8.download.listener.DownloadListener;
import com.lt.m3u8.download.utils.Constant;
import com.lt.m3u8.download.utils.Log;
import com.lt.m3u8.download.format.MediaFormat;
import com.lt.m3u8.utils.StringUtils;
...
public class M3u8DownloadFactory {
	...
	public static class M3u8Download {
		...
		// 优化内存占用
		private static BlockingQueue<byte[]> BLOCKING_QUEUE = new LinkedBlockingQueue<>();
		...
		// 所有ts片段下载链接
		private static Set<String> tsSet = new LinkedHashSet<>();

		// 解密后的片段
		private Set<File> finishedFiles = new ConcurrentSkipListSet<>(
				Comparator.comparingInt(o -> Integer.parseInt(o.getName().replace(".xyz", ""))));
...
//								.multiply(new BigDecimal(100)).setScale(2, BigDecimal.ROUND_HALF_UP) + "%");
				...
				Log.i("下载完成，正在合并文件！共" + finishedFiles.size() + "个！"
		 ...
		 * 合并下载好的ts片段
		 */
		private synchronized void mergeTs() {
			File file = new File(dir + FILESEPARATOR + fileName + ".ts");
			try {
				if (file.exists())
					file.delete();
				else
		...
		private Thread getThread(String urls, int i) {
			return new Thread(() -> {
				...
				File file2 = new File(tsdir + FILESEPARATOR + i + ".xy");
				if (file2.exists())
					file2.delete();
				...
				while (count != 0) {
					try {
						...
						File file = new File(tsdir + FILESEPARATOR + i + ".xyz");
		...
		private String getTsUrl() {
			...
			if (!content.toString().contains("#EXTM3U"))
				throw new RuntimeException(DOWNLOADURL + "不是m3u8链接！");
			...
			if (StringUtils.isEmpty(keyUrl))
				throw new RuntimeException("未发现key链接！");
			// 获取密钥
		...
		private String getKey(String url, StringBuilder content) {
			...
			if (!StringUtils.isEmpty(key)) {
				key = key.replace("\"", "");
				String contentStr ="";
				if(StringUtils.isUrl(key)) {
					contentStr = key;
				}else {
					contentStr =  mergeUrl(relativeUrl, key);
				}
				//StringUtils.isUrl(key) ? key : mergeUrl(relativeUrl, key)
				return getUrlContent(contentStr, true).toString()
						.replaceAll("\\s+", "");
			}
			return null;
		}
		...
		private StringBuilder getUrlContent(String urls, boolean isKey) {
			...
			while (count != 0 && count < retryTimes) {
				try {
					URL url = new URL(urls);
					if (proxy == null) {
						httpURLConnection = (HttpURLConnection) url.openConnection();
		...
		private void checkField() {
			if ("m3u8".compareTo(MediaFormat.getMediaFormat(DOWNLOADURL)) != 0)
				throw new RuntimeException(DOWNLOADURL + "不是一个完整m3u8链接！");
			if (threadCount <= 0)
				throw new RuntimeException("同时下载线程数只能大于0！");
			if (timeoutMillisecond < 0)
				throw new RuntimeException("超时时间不能小于0！");
			if (StringUtils.isEmpty(dir))
				throw new RuntimeException("视频存储目录不能为空！");
			if (StringUtils.isEmpty(fileName))
				throw new RuntimeException("视频名称不能为空！");
			finishedCount = 0;
			method = "";
			key = "";
			isByte = false;
		...
		public void setDir(String dir) {
			this.dir = dir;
			this.tsdir = dir + File.separator + "temp";
		}
